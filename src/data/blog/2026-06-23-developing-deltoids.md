...
description: "Building Deltoids with Claude: vibe-slop, edge cases, legacy code, and the algorithm I had to feel my way toward. The tools changed; the feeling didn't."
...
But here's a different way to look at it: Could I have sat down and developed an algorithm that solves this well? I don't think I could have. Before AI, I would have tried anyway. I would probably have written a basic version, maybe just as bad as the AI's, maybe a little better, who knows. But **it would have taken a long, long time.**

As the edge cases appeared, I wrote tests and fixed individual cases. Claude and I never redesigned a solution. Code quality went down. The more this happened, the more other things broke, until **it got really hard to add anything at all.** This is not specific to agents. **This is just how coding works.**

Now this code is a legacy system, and I love legacy systems. They hold all the knowledge you need to build something great. You only have to read the code and figure out what algorithm would actually cover all the cases. You have to generalize it. And in the process of covering those edge cases, asking Claude to fix them, and reviewing the code, **I developed a feeling for the algorithm.** What is that feeling if not experience grounded in years of software development? That's important to me: even working through Claude, I still get the feeling for the algorithm I am after.

Now I have tests, **feedback, experience, and real usage data** to check that the feeling is pointing in the right direction. I can feed all of that into Claude and design the algorithm along with it.
